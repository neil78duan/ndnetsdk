...
int timer_func2(void *param)
{
	ndprintf(_NDT("timer function \n exit host")) ;
	nd_host_eixt() ;
	return 0;
}

int timer_func(void *param)
{
	static int s1 = 0 ;
	ndprintf("timer start %d \n", s1++) ;
	if(s1==10){
		//nd_thsrv_timer_del(__g_timer, 0) ;
		printf("timer %d is deleted \n") ;
		__g_timer = nd_timer_add(__s_timer,timer_func2, NULL,1000,ETT_LOOP) ;
		return -1 ;
	}
	return 0 ;
}
...
int sender(void *p) 
{
	...
	if(0==times){
		__g_timer = nd_timer_add(__s_timer, timer_func, NULL,1000,ETT_LOOP) ;
		if (!__g_timer)	{
...
{
	static int recved = 0 ;
	ndfprintf(stderr, _NDT(" received message from %d\n datalen=%d data=%s\n"), 
...
//	nd_handle context ;
	...
	__s_timer = nd_timer_create(nd_global_mmpool()) ;
	if(!__s_timer) {
		ndprintf("create sender timer error \n") ;
		return -1 ;
	}
	__r_timer = nd_timer_create(nd_global_mmpool()) ;
	if(!__s_timer) {
		ndprintf("create receiver timer error \n") ;
		return -1 ;
	}
//	nd_common_init() ;
