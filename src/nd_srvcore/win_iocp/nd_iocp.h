...
static __INLINE__ NDUINT16 iocp_session_id(struct nd_client_map_iocp *iocp_map)
{
	return iocp_map->__client_map.connect_node.session_id  ;
}

/*
static __INLINE__ int check_send_list_empty(struct nd_client_map_iocp *iocp_map)
{
	return iocp_map->wait_send_list.next == &iocp_map->wait_send_list ;
}
*/
static __INLINE__ int iocp_unnotified_length(struct nd_client_map_iocp *iocp_map)
{
	return (int)(iocp_map->total_send - iocp_map->send_len) ;
}
static __INLINE__ nd_netbuf_t * iocp_send_buf(struct nd_client_map_iocp *cli_map)
{
	return &(cli_map->__client_map.connect_node.send_buffer) ;
}

static __INLINE__ nd_netbuf_t * iocp_recv_buf(struct nd_client_map_iocp *cli_map)
{
...
//int iocp_parse_msgex(struct nd_client_map_iocp *iocp_map,NDNET_MSGENTRY msg_entry );
